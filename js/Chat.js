    ...
    channelEvent.onMessageReceived = (channel, message) => {
        if(window.Notification) {
                Notification.requestPermission(function(status) {
                    ...
                    /** consider adding additional logic:   
                     *  check message type (handle differently for file transfer/media vs. text msg
                     *  check user details - user nickname and profile picture, adding the 2 properties into notification)
                     */ 

                    var msg = message.message;
                    var n = new Notification('Sendbird New Message', { 
                      body: msg,
                      tag: channel.url
                   });
                   n.onclick = function(){
